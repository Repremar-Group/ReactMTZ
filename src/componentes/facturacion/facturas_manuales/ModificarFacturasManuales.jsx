...
const ModificarFacturasManuales = ({ isOpen, onClose, onFinally, onSuccess, onError, factura }) => {
  ...
  //Carga de la factura cuando se habre el modal
  useEffect(() => {
    if (factura) {
      setFmId(factura.factura.IdCliente || '');
      setSearchTerm(factura.factura.Nombre || '');
      setCodigoClienteGIA(factura.factura.CodigoClienteGia || '');
      setFmComprobanteElectronico((factura.factura.ComprobanteElectronico || ''));
      setFmCiudad(factura.factura.Ciudad || '');
      setFmPais(factura.factura.Pais || '');
      setFmRazonSocial(factura.factura.RazonSocial || '');
      setFmTipoIva(factura.factura.TipoIVA || '');
      setFmMoneda(factura.factura.Moneda || 'USD');
      const fechaOriginal = factura.factura.Fecha;
      const fechaFormateada = fechaOriginal ? new Date(fechaOriginal).toISOString().split('T')[0] : '';
      setFmFecha(fechaFormateada);
      const fechaOriginalVenc = factura.factura.fechaVencimiento;
      const fechaFormateadaVenc = fechaOriginal ? new Date(fechaOriginalVenc).toISOString().split('T')[0] : '';
      setFmFechaVencimiento(fechaFormateadaVenc);
      setFmDireccionFiscal(factura.factura.DireccionFiscal || '');
      setFmRutCedula(factura.factura.RutCedula || '');
      setFmCass(factura.factura.CASS || '');
      setFmTipoDeEmbarque(factura.factura.TipoEmbarque);
      setFmTc(factura.factura.TC)
      const detallesDesdeDB = factura.detalles.map((d) => ({
        codigo: d.Codigo,
        descripcion: d.Descripcion,
        importe: d.Importe.toString(), // si querés mantenerlo como string
        impuesto: d.impuesto,
        ivaCalculado: d.IVA.toString(), // si lo usás como string en el front
        moneda: d.Moneda,
        codigoGIA: d.codigoGIA,
        // completá con los campos nulos si los necesitás
        clasificacion: null,
        categoria: null,
      ...
      // 2. Reemplazar en el estado
      setFmListaDeConceptos(detallesDesdeDB);
    }
  }, [factura]);
  ...
  const handleActualizarFacturaManual = async () => {
    ...
    try {
      const response = await axios.put(`${backURL}/api/modificarFacturaManual`, datosFormulario);
  ...
  return (
    <div onClick={onClose} className="modal">
      <div className="EmitirFacturaManual-container" onClick={(e) => e.stopPropagation()}>
        <ToastContainer />
        {loading && (
        ...
        <form className='formulario-estandar'>
              ...
              <div className='div-renglon-datos-facturasmanuales'>
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="FCD">E-Factura</option>
                    <option value="TCD">E-Ticket</option>
                    <option value="FCA">E-Factura Cuenta Ajena</option>
                    <option value="TCA">E-Ticket Cuenta Ajena</option>
                  </select>
          ...
          <div className='botonesemitircomprobante'>
            <button type="button" className='btn-facturar' onClick={handleActualizarFacturaManual}>Guardar</button>

            <button type="button" className="btn-estandar" onClick={onClose}>Volver</button>
          </div>
...
export default ModificarFacturasManuales